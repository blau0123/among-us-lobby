...
Lobby::Lobby(std::string filename, int bfCull, int useTexture, int useToonShading)
...
void Lobby::initRotateModel(int windowWidth, int windowHeight, glm::vec2 cursorPos) {
...
void Lobby::rotateModel(int windowWidth, int windowHeight, glm::vec2 currCursorPos) {
...
void Lobby::endRotateModel() {
	// Turn off rotation
	movement = -1;
}

// Calculate 3D position of a projected unit vector onto the xy-plane
glm::vec3 Lobby::trackBallMapping(int windowWidth, int windowHeight, glm::vec2 cursorPos) {
	glm::vec3 v;
	float d;
